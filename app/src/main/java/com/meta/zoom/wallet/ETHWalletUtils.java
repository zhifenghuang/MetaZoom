...
//        System.out.println(Arrays.toString(seedBytes));
        ...
        WalletBean ethWallet = generateWallet(walletName, pwd, keyPair);
        if (ethWallet != null) {
            ethWallet.setMnemonic(convertMnemonicList(mnemonic));
            ethWallet.setMnemonicCode(mnemonic);
        }
        return ethWallet;
    }
    ...
    private static WalletBean generateWallet(String walletName, String pwd, ECKeyPair ecKeyPair) {
        ...
        LogUtil.LogE("publicKey = " + s);

        String wallet_dir = AppFilePath.Wallet_DIR;
        LogUtil.LogE("wallet_dir = " + wallet_dir);
        ...
        WalletBean ethWallet = new WalletBean();
        ethWallet.setWalletName(walletName);
     ...
     * @return
     */
    public static WalletBean loadWalletByKeystore(String keystore, String pwd) {
...
//            ToastUtils.showToast(R.string.load_wallet_by_official_wallet_keystore_input_tip);
     ...
     * @return
     */
    public static WalletBean modifyPassword(int walletId, String walletName, String oldPassword, String newPassword) {
        WalletBean ethWallet = DatabaseOperate.getInstance().getWalletById(walletId);
        Credentials credentials = null;
     ...
     * @return
     */
    public static String derivePrivateKey(int walletId, String pwd) {
        WalletBean ethWallet = DatabaseOperate.getInstance().getWalletById(walletId);
     ...
     * @return
     */
    public static String deriveKeystore(int walletId, String pwd) {
        WalletBean ethWallet = DatabaseOperate.getInstance().getWalletById(walletId);
     ...
     * @return
     */
    public static boolean deleteWallet(int walletId) {
        WalletBean wallet = DatabaseOperate.getInstance().getWalletById(walletId);
        if (deleteFile(wallet.getKeystorePath())) {
            DatabaseOperate.getInstance().delete(wallet);
            return true;
        } else {
            return false;
        }
