...
public class WebSocketHandler {
    ...
    private static WebSocketManager orderWebSocket;
...
//        if(defaultWebSocket==null){
//            ChatManager.getInstance().initWebSocket(DataManager.getInstance().getToken());
//        }
        return defaultWebSocket;
    }
    ...
    private static boolean mIsRegisterNetworkChangedReceiver;
    ...
    public static void registerNetworkChangedReceiver(Context context) {
        if (PermissionUtil.checkPermission(context, Manifest.permission.ACCESS_NETWORK_STATE) && !mIsRegisterNetworkChangedReceiver) {
            try {
                IntentFilter filter = new IntentFilter();
                filter.addAction(ConnectivityManager.CONNECTIVITY_ACTION);
                context.registerReceiver(new NetworkChangedReceiver(), filter);
                mIsRegisterNetworkChangedReceiver = true;
            } catch (Exception e) {
